...
class GameStateManager:
    ...
    def execute_move(
        ...
        self._update_piece_lists(piece, to_square, promotion_piece, is_capture, is_en_passant, captured_piece)
    ...
    def _update_piece_lists(
        ...
        """Update white_pieces and black_pieces arrays"""
        is_white = "-" not in piece
        
        # Remove captured piece
        if is_capture:
            if is_en_passant:
                # En passant: remove pawn from the piece list
                captured_pawn = "-p" if is_white else "p"
                # Find and remove any pawn matching this color
                if is_white:
                    # We captured a black pawn
                    self.database.black_pieces = self.database.black_pieces[self.database.black_pieces != captured_pawn]
                else:
                    # We captured a white pawn
                    self.database.white_pieces = self.database.white_pieces[self.database.white_pieces != captured_pawn]
            else:
                # Normal capture: remove the captured piece we saved earlier
                if captured_piece:
                    if is_white:
                        # We captured a black piece
                        self.database.black_pieces = self.database.black_pieces[self.database.black_pieces != captured_piece]
                    else:
                        # We captured a white piece
                        self.database.white_pieces = self.database.white_pieces[self.database.white_pieces != captured_piece]
