...
class NotationGenerator:
    ...
    def generate_fen(
        ...
        return f"{board_fen} {side_to_move[0]} {castling} {en_passant} {halfmove_clock} {fullmove_number}"
    ...
    def chess_notation(
        ...
        if castle:
            ...
            if piece_type == "p":
                ...
                notation = (
                    f"{piece_letter}{disambiguation}{capture_marker}{to_notation}"
                )
...
class LegalMovesEngine:
    ...
    @staticmethod
    def search_piece(
        piece: str, matrix: Optional[np.ndarray] = None
    ) -> Tuple[int, int]:
        """
    ...
    def find_pins(
        self, color: Literal["white", "black"], matrix: np.ndarray
    ...
    def _rook_blocking_squares(
        self, rook_pos: Tuple[int, int], king_pos: Tuple[int, int]
    ) -> List[Tuple[int, int]]:
    ...
    def _bishop_blocking_squares(
        self, bishop_pos: Tuple[int, int], king_pos: Tuple[int, int]
    ) -> List[Tuple[int, int]]:
    ...
    def can_castle(
        ...
        if self._is_square_attacked_by_any_piece(
            (row, king_col), opponent_color, matrix
        ):
            return False
    ...
    def _generate_sliding_moves(
        ...
        for dr, dc in directions:
            ...
            if pin_direction:
                if (dr, dc) != pin_direction and (dr, dc) != (
                    -pin_direction[0],
                    -pin_direction[1],
                ):
                    continue
    ...
    def rook_moves(
        self, piece: str, matrix: Optional[np.ndarray] = None
    ) -> List[Tuple[int, int]]:
        ...
        moves = self._generate_sliding_moves(
            piece, position, STRAIGHT_DIRECTIONS, matrix, pin_direction
        )
    ...
    def bishop_moves(
        ...
        moves = self._generate_sliding_moves(
            piece, position, DIAGONAL_DIRECTIONS, matrix, pin_direction
        )
        moves = self.check_allowed_moves(moves, color, matrix)
    ...
    def knight_moves(
        ...
        for dr, dc in KNIGHT_OFFSETS:
            ...
            if self.is_valid_square(r, c):
                if not self.is_occupied(matrix[r, c]) or self.is_enemy(
                    matrix[r, c], color
                ):
                    moves.append((r, c))
    ...
    def king_moves(
        ...
        if color == "white":
            castle_checks = [
                (database.r1_moved, database.k1_moved),
                (database.r2_moved, database.k1_moved),
            ]
            rook_pieces = ["r1", "r2"]
        else:
            castle_checks = [
                (database.r1_black_moved, database.k1_black_moved),
                (database.r2_black_moved, database.k1_black_moved),
            ]
            rook_pieces = ["-r1", "-r2"]
        ...
        for idx, (castle_range, castle_col) in enumerate(
            zip(castle_ranges, castle_cols)
        ):
        ...
        safe_moves = self._filter_king_moves_by_safety(
            moves, piece, row, col, opponent_color, matrix
        )
        return safe_moves
    ...
    def _filter_king_moves_by_safety(
        ...
        for move in moves:
            ...
            is_safe = not self._is_square_attacked_by_any_piece(
                move, opponent_color, temp_matrix
            )
    ...
    def _is_clear_diagonal(
        self, from_pos: Tuple[int, int], to_pos: Tuple[int, int], matrix: np.ndarray
    ) -> bool:
...
class AIUtilities:
    ...
    def configure_strength(self, elo: int) -> None:
        ...
        database.gamelogger.ai(
            f"Configured: {config.description} ({config.elo} ELO) | Skill: {config.skill_level} | Depth: {config.depth} | Blunder: {config.blunder_chance * 100:.0f}%"
        )
        ...
        try:
            ...
            if config.blunder_chance > 0:
                if random.random() < config.blunder_chance:
                    ...
                    database.gamelogger.ai(
                        f"Blundering! ({database.ai_blunder_moves}/{database.ai_total_moves} = {pct:.0f}%)"
                    )
    ...
    def _get_weighted_move(self, config: StockfishConfig) -> Optional[str]:
        ...
        if len(moves) > 1:
            database.gamelogger.ai(
                f"Pick #{moves.index(selected_move) + 1}/{len(moves)}"
            )
    ...
    def update_start_fen() -> None:
        ...
        if database.stockfish:
            database.stockfish.set_fen_position(
                "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"
            )
            if database.game_history:
                database.stockfish.make_moves_from_current_position(
...
class Utilities:
    ...
    def ctkimage_generator(
        self, path: str, size: Tuple[int, int] = (70, 70)
    ) -> ctk.CTkImage:
    ...
    def create_selector_frame(
        ...
        content_frame.pack(
            fill="both", expand=True, padx=int(width * 0.05), pady=int(height * 0.13)
        )
...
if __name__ == "__main__":
    utils = Utilities()
    print(
        utils.notations.generate_matrix(
            "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"
        )
    )
